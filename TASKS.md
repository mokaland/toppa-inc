...
| Q1-T5 | ユーザー認証の追加 | Tech Lead (マルコ) | 2026-03-14 | **ブロック中** | `src/components/Login.tsx`の作成を確認。CIビルド失敗のため現在ブロック中。 |
| **Q1-T5-FIX** | **CIビルド失敗の緊急修正 (`Login.tsx`)** | **Tech Lead (マルコ)** | **2026-02-20** | **ブロッカー** | **ビルド失敗が継続中。インラインstyleが原因と推測したが、コード上からは発見できず。Tech Leadは直ちに真の原因を特定し、修正すること。修正後、原因と対策をSlackで報告せよ。これが解決するまで他タスクは進行不可。** |
...
2. **`TASKS.md` の指示に従い、`Q1-T5-FIX` を本日中に完了させよ。**
...
8. **【最重要】Reactコンポーネント内でインラインの`<style>`タグや`@keyframes`定義は使うな。** ビルドが失敗する原因になる。過去に2度同じ問題が発生している。アニメーションは`tailwind.config.js`に定義するか、`animate-pulse`のような既存クラスを使え。
