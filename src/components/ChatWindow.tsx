...
const ChatWindow: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 p-4">
      {/* メッセージ表示エリア */}
      <div className="flex-grow overflow-y-auto mb-4 p-4 bg-white rounded-lg shadow-inner">
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-start mb-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && (
              <div className="w-8 h-8 rounded-full bg-neutral-600 flex items-center justify-center font-bold text-white text-sm mr-3">
                AI
              </div>
            )}
            <div className={`max-w-[70%] p-3 rounded-lg shadow-md break-words ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-neutral-200 text-neutral-800'}`}>
              <div className="text-xs text-neutral-400 mb-1">{msg.timestamp}</div>
              <p>{msg.content}</p>
            </div>
            {msg.role === 'user' && (
              <div className="w-8 h-8 rounded-full bg-primary-dark flex items-center justify-center font-bold text-white text-sm ml-3">
                You
              </div>
            )}
          </div>
        ...
        {isLoading && (
          <div className="flex items-start mb-4 justify-start">
            ...
            <div className="max-w-[70%] p-3 rounded-lg bg-neutral-200 text-neutral-800 shadow-md">
              <SpinnerIcon />
            </div>
          </div>
        )}
      ...
      <div className="flex-shrink-0 flex">
        ...
        <button
          onClick={sendMessage}
          className="bg-primary text-white p-3 rounded-r-lg hover:bg-primary-dark disabled:bg-primary-light"
